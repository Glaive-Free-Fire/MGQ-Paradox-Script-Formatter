...
// Функция для проверки строки на ID (включая диапазоны вида "2-3" и списки диапазонов)
function isIDLine(line) {
  // Проверяем различные форматы ID:
  // - Просто число: "123"
  // - Диапазон: "123-456"
  // - Списки и смеси: "123, 456-458, 500"
  // - С комментарием: "123 #комментарий" или "123-456 #комментарий"
  return line.trim().match(/^(\d+(?:-\d+)?)(?:,\s*\d+(?:-\d+)?)*(\s*#.*)?$/);
}
...
function processLibraryFile(content, newBlocks, isDirectCall) {
  try {
    ...
    const dictStartIndex = offset; // Начинаем с начала строки, чтобы не дублировать название
    ...
    if (dictEndLine === -1) {
      ...
      if (bestMatch) {
        ...
        return processDictionaryContent(dictStartIndex, dictEndIndex, dictionaryContent, content, newBlocks, isDirectCall, lines, dictionaryStartLine, dictEndLine);
      }
    ...
    // Вычисляем позицию конца словаря. Используем indexOf для надежности в разных ОС
    const dictEndIndex = content.indexOf(lines[dictEndLine], offset) + lines[dictEndLine].length;
...
function processWithRegex(content, newBlocks, isDirectCall, dictStartPosition) {
  ...
  if (bestMatch) {
    ...
    const lines = content.split('\n');
    // Пытаемся примерно определить индексы строк для совместимости
    const dictionaryStartLine = content.substring(0, dictStartPosition).split('\n').length - 1;
    const dictEndLine = content.substring(0, dictEndIndex).split('\n').length - 1;

    return processDictionaryContent(dictStartPosition, dictEndIndex, dictionaryContent, content, newBlocks, isDirectCall, lines, dictionaryStartLine, dictEndLine);
  }
...
function processDictionaryContent(dictStartIndex, dictEndIndex, dictionaryContent, content, newBlocks, isDirectCall, lines, dictionaryStartLine, dictEndLine) {
  // Определяем формат окончания словаря - используем оригинальную строку
  const dictEndFormat = lines[dictEndLine] || "} # ENEMY_DESCRIPTION";
  ...
  // Используем оригинальную строку до открывающей скобки включительно
  const dictionaryLine = lines[dictionaryStartLine];
  const openBracePos = dictionaryLine.indexOf('{');
  let updatedContent = content.substring(0, dictStartIndex);
  updatedContent += dictionaryLine.substring(0, openBracePos + 1);
  ...
  let remainingContent = content.substring(dictEndIndex);

  // Добавляем оригинальную закрывающую строку
  updatedContent += "\n" + dictEndFormat;
